...
def create_all_files(number_of_test_runs, is_commited=False):
    ...
    for random_seed in range(number_of_test_runs):
        for non_faulty_count in range(10,20):
            ...
            tmp_file_path, _ = create_experiment_file(random_seed, non_faulty_count, faulty_count, is_commited=is_commited)
...
def main():
    ...
    tp = ThreadPool(num)
    build()
    file_paths1 = create_all_files(number_of_test_runs=50, is_commited=True)
    file_paths2 = create_all_files(number_of_test_runs=50, is_commited=False)
    for tmp_file_path in file_paths1+file_paths2:
